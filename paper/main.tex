...
\author{Aadrika Deokathe\inst{1} \and Khushi\inst{1} \and [Faculty Guide Name]\inst{1}}
\authorrunning{A. Deokathe et al.}

\institute{Department of Computer Engineering, [University Name], [City], India\\
\email{\{aadrikadeokathe, khushi\}@example.com}}
...
Ablating support prompt cardinality across $K \in \{1, 2, 4, 8\}$ shots reveals near-instant saturation ($K=1$: $0.4001$, $K=8$: $0.3997$; Fig.~\ref{fig:shot_ablation}). This indicates that the 3-layer cross-attention adapter extracts the necessary semantic guidance from a single visual exemplar, avoiding the clinical burden of annotating multi-slice reference volumes.
