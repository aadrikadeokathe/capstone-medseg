...
\begin{table}[t]
\centering
\caption{Cross-Anatomical Zero-Shot Generalization: Standard Per-Dataset Training vs. Episodic Multi-Task Meta-Learning (2-shot). Standard training collapses ($3.99\%$ average Dice) due to the Spatial Memorization Trap. Multi-task episodic meta-learning forces invariant semantic prompt matching, yielding an average $+35.99$ percentage point ($10\times$) gain across unseen targets.}
\label{tab:zeroshot_episodic}
\begin{tabular}{lcccc}
\toprule
\textbf{Target Dataset (Modality)} & \textbf{Standard Zero-Shot} & \textbf{UniverSeg Base} & \textbf{Episodic Zero-Shot} & \textbf{Absolute Gain} \\
\midrule
Spleen (CT)           & $1.47\%$  & $55.80\%$ & $\mathbf{15.65\%}$ & $+14.18\text{ pp}$ \\
Liver (CT)            & $1.04\%$  & $49.50\%$ & $\mathbf{62.48\%}$ & $\mathbf{+61.44\text{ pp}}$ \\
Heart (Cine MRI)      & $10.68\%$ & $31.20\%$ & $\mathbf{42.29\%}$ & $+31.61\text{ pp}$ \\
Brain Tumour (MRI)    & $2.78\%$  & $16.10\%$ & $\mathbf{39.52\%}$ & $+36.74\text{ pp}$ \\
\midrule
\textbf{Average}      & $\mathbf{3.99\%}$ & $38.15\%$ & $\mathbf{39.98\%}$ & $\mathbf{+35.99\text{ pp}}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/zeroshot_transfer_comparison.png}
\caption{Zero-Shot Cross-Anatomical Transfer Comparison between Standard In-Domain Training and Episodic Multi-Task Meta-Learning across Four Medical Image Benchmarks. Episodic training prevents cross-attention from memorizing fixed spatial coordinates, achieving an average tenfold transfer gain (+35.99 pp).}
\label{fig:zeroshot_transfer}
\end{figure}

\subsection{Cross-Domain Generalization and the Spatial Memorization Trap}
When evaluating standard in-domain adapters on unseen anatomical organs (zero-shot transfer), performance drops catastrophically to $0.01\text{--}0.04$ Dice (Table~\ref{tab:zeroshot_episodic} and Fig.~\ref{fig:zeroshot_transfer}). Cross-attention heatmaps (Fig.~\ref{fig:attention}) reveal that the adapter learns fixed spatial priors during sequential batch training, ignoring support mask prompts. In contrast, our multi-task episodic meta-learning enforces invariant semantic token matching, elevating zero-shot transfer by $10\times$ ($3.99\% \rightarrow 39.98\%$) and surpassing the foundation baseline on Liver ($62.48\%$ vs. $49.50\%$), Heart ($42.29\%$ vs. $31.20\%$), and Brain Tumour ($39.52\%$ vs. $16.10\%$).
