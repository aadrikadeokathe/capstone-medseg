...
def get_heart_splits(
    ...
    if not os.path.exists(images_path) or not os.path.exists(masks_path):
        from src.preprocessing import build_heart_dataset
        build_heart_dataset(output_dir=data_dir)
...
def get_liver_splits(
    ...
    """
    Loads liver images and masks, splits into train/val/test sets, and creates InContextDataset instances.
    Uses a 60/20/20 split ratio (train_ratio=0.60, val_ratio=0.20, test_ratio=0.20) tailored for Liver's small
    57-slice dataset size to ensure sufficient validation and test evaluation samples (34 train / 11 val / 12 test).
    """
    images_path = os.path.join(data_dir, "liver_images.npy")
    masks_path = os.path.join(data_dir, "liver_masks.npy")

    if not os.path.exists(images_path) or not os.path.exists(masks_path):
        raise FileNotFoundError(f"Processed liver data not found at {images_path} and {masks_path}")
